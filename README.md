...
<b>2. 制限事項</b><BR>
このボードは簡素なつくりのため、FDS+と比べ機能制限が多いです。エミュレーションモードでのドライブ番号(0のみ、1のみ、0/1両方)はジャンパーで設定するようになっており、動的な変更はできません(もちろん、このジャンパーのピンヘッダ―に双極双投のトグルスイッチなどを接続すれば外部から切り替えることは可能です)。また、PC-98における2DDディスクの回転数が他のシステムと異なるため、2DDディスクの扱いにも制限があります。<br><br>
...
<b>11. 既知の問題点</b><br>
概ね設計どおりの動作を確認してはいますが、 こちらの動作確認環境(PC-9801BX3+OSD)では1.44MBモードでの動作がやや不安定です。具体的には、エミュレーションモードにて1.44MBのイメージをマウントし、そのイメージに書き込むと読めないセクターができてしまうことがありました。また、1.44MBのディスクのダンプは特に問題なくできましたが、リストアを行うと一部にエラーのあるディスクができてしまうことがありました。このボードの設計というよりは、FDDやマザーボードとFDX68の相性なのかもしれないと考えています。<br>
また、FDX68は本来電源OFFの状態で接続されることを想定していません。環境によっては動作が不安定になる(FDDが本体から4台見えてしまう、3モードFDDの動作がおかしくなる)場合があることを確認しています。これはおそらくFDX68シールドのプルアップ抵抗(ターミネーション抵抗)を経由してシールド上のロジックICに中途半端な電圧で給電されてしまうことが原因ではないかと考えています。対策としてはプルアップ抵抗(RN1-3)を装着しないか、コモン端子にダイオードを挟んで接続すると解決できる可能性があります。こちらでも試しにプルアップ抵抗を除去してみたところ、上記のような現象は見られなくなりました。ただしこの場合、ケーブルは極力短くしなければ動作が不安定になる可能性があります。<br><br>
